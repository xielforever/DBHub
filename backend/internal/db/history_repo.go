...
// HistoryBrief 查询历史简要条目（全局搜索用）。
type HistoryBrief struct {
	ID             int64     `json:"id"`
	ConnectionID   int64     `json:"connection_id"`
...
// SearchBySQL 按关键字检索当前用户可见的成功查询历史（管理员可见全部）。
func (r *HistoryRepository) SearchBySQL(ctx context.Context, userID int64, isAdmin bool, keyword string, limit int) ([]HistoryBrief, error) {
	if limit <= 0 || limit > 20 {
		limit = 8
	}
	rows, err := r.pool.Query(ctx, `
		SELECT h.id, h.connection_id, COALESCE(c.name, ''),
		       COALESCE(h.database_name, ''), h.sql_text, h.created_at
		FROM sys_query_history h
		LEFT JOIN sys_connections c ON c.id = h.connection_id
		WHERE h.status = 1
		  AND ($2 OR h.user_id = $1)
		  AND h.sql_text ILIKE $3
		ORDER BY h.id DESC
		LIMIT $4`, userID, isAdmin, "%"+keyword+"%", limit)
	if err != nil {
		return nil, fmt.Errorf("检索查询历史失败: %w", err)
	}
	defer rows.Close()
	out := make([]HistoryBrief, 0, limit)
	for rows.Next() {
		var b HistoryBrief
		if err := rows.Scan(&b.ID, &b.ConnectionID, &b.ConnectionName, &b.DatabaseName, &b.SQLText, &b.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, b)
	}
